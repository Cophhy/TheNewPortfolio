...
export default function CareerExperience() {
  ...
  // idioma atual
  const { lang } = useLang();
  const items = getCareer(lang);

  // título por idioma
  const TITLE =
    lang === "pt" ? "Minha carreira & experiência" : "My career & experience";
  ...
  const stretched = useTransform(scrollYProgress, [0, 0.985], [0, 1], {
    clamp: true,
  });
  const progress = useSpring(stretched, {
    stiffness: 120,
    damping: 20,
    mass: 0.3,
  });
  ...
  // zera/realoca refs quando a quantidade de itens mudar (i18n ou dados)
  useEffect(() => {
  ...
  }, [items.length]); // re-medir se mudar a lista (troca de idioma)
      ...
      <div ref={trackRef} className="relative">
        ...
        >
          <motion.div
            ...
            className="origin-top relative h-full w-px"
          >
